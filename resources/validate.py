...
def validate(date, temp, prec, prec_type, wind, wind_dir, humi, airp, clou):
    ...
    elif not re.compile("^\d+(\.\d+)?$").match(temp):
    ...
    elif prec_type not in ["None", "Rain", "Snow", "Hail", "Sleet"]:
    ...
    elif wind_dir not in ["None", "N", "NNE", "NE", "ENE", "E", "ESE", "SE", "SSE", "S", "SSW", "SW", "WSW", "W", "WNW", "NW", "NNW"]:
    ...
    elif not re.compile("^\d+(\.\d+)?$").match(humi):
    ...
    elif not re.compile("^\d+(\.\d+)?$").match(airp):
